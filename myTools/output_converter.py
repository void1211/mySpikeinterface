...
def convert_to_matlab_files_ks4(res_dir: Path | str, recording):
    ...
    ----------
    res_dir : Path | str
        結果フォルダ（`spike_times.npy` などがあるディレクトリ）。
    recording : Recording
        SpikeInterface の Recording オブジェクト。
    """
    ks_dir = Path(res_dir) / 'sorting' / 'sorter_output'
    save_dir = Path(res_dir) / 'matlab'
    save_dir.mkdir(parents=True, exist_ok=True)

    amplitudes = safe_load_npy(ks_dir / "amplitudes.npy")
    channel_map = safe_load_npy(ks_dir / "channel_map.npy")
    channel_positions = safe_load_npy(ks_dir / "channel_positions.npy")
    channel_shanks = safe_load_npy(ks_dir / "channel_shanks.npy")
    cluster_Amplitude = safe_load_csv(ks_dir / "cluster_Amplitude.tsv", sep="\t")
    cluster_ContamPct = safe_load_csv(ks_dir / "cluster_ContamPct.tsv", sep="\t")
    cluster_group = safe_load_csv(ks_dir / "cluster_group.tsv", sep="\t")
    cluster_KSLabel = safe_load_csv(ks_dir / "cluster_KSLabel.tsv", sep="\t")
    kept_spikes = safe_load_npy(ks_dir / "kept_spikes.npy")
    from kilosort.io import load_ops
    ops = load_ops(ks_dir / "ops.npy")
    pc_features_ind = safe_load_npy(ks_dir / "pc_feature_ind.npy")
    pc_features = safe_load_npy(ks_dir / "pc_features.npy")
    similar_templates = safe_load_npy(ks_dir / "similar_templates.npy")
    spike_clusters = safe_load_npy(ks_dir / "spike_clusters.npy")
    spike_detection_templates = safe_load_npy(ks_dir / "spike_detection_templates.npy")
    spike_positions = safe_load_npy(ks_dir / "spike_positions.npy")
    spike_templates = safe_load_npy(ks_dir / "spike_templates.npy")
    spike_times = safe_load_npy(ks_dir / "spike_times.npy")
    templates = safe_load_npy(ks_dir / "templates.npy")
    templates_ind = safe_load_npy(ks_dir / "templates_ind.npy")
    whitening_mat = safe_load_npy(ks_dir / "whitening_mat.npy")
    whitening_mat_data = safe_load_npy(ks_dir / "whitening_mat_dat.npy")
    whitening_mat_inv = safe_load_npy(ks_dir / "whitening_mat_inv.npy")
    ...
    savemat(save_dir / 'results.mat', results_dict, long_field_names=True, format='5')
    replaced_ops = replace_none_recursive(ops)
    savemat(save_dir / 'ops.mat', {'ops': replaced_ops}, long_field_names=True)


def convert_to_matlab_files(res_dir: Path | str, recording):
    """Phy フォルダ内のファイルを使って .mat を作成する関数。"""

    phy_dir = Path(res_dir) / 'phy'
    analyzer_dir = Path(res_dir) / 'analyzer' / 'extensions'
    save_dir = Path(res_dir) / 'matlab'
    save_dir.mkdir(parents=True, exist_ok=True)

    amplitudes = safe_load_npy(phy_dir / "amplitudes.npy")
    spike_times = safe_load_npy(phy_dir / "spike_times.npy")
    spike_positions = safe_load_npy(analyzer_dir / 'spike_locations' / "spike_locations.npy")
    pc_features = safe_load_npy(phy_dir / "pc_features.npy")
    pc_features_ind = safe_load_npy(phy_dir / "pc_feature_ind.npy")
    ...
    cluster_label = safe_load_csv(phy_dir / "cluster_label.tsv", sep="\t")
    cluster_ContamPct = safe_load_csv(phy_dir / "cluster_contamPct.tsv", sep="\t")
    cluster_Amplitude = safe_load_csv(phy_dir / "cluster_amplitude.tsv", sep="\t")
    spike_clusters = safe_load_npy(phy_dir / "spike_clusters.npy")
    spike_templates = safe_load_npy(phy_dir / "spike_templates.npy")
    kept_spikes = safe_load_npy(phy_dir / "kept_spikes.npy")
    whitening_mat = safe_load_npy(phy_dir / "whitening_mat.npy")
    whitening_mat_data = safe_load_npy(phy_dir / "whitening_mat_dat.npy")
    whitening_mat_inv = safe_load_npy(phy_dir / "whitening_mat_inv.npy")
